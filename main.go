...
func main() {
	...
	user_dao := dao.NewUserDao(db, "gym-log", "users")
	user_handler := handlers.NewUserHandler(user_dao)
	routers.RegisterUserRoutes(user_mux, user_handler)
	...
	mainMux.Handle("/user", http.StripPrefix("/user", user_mux))
